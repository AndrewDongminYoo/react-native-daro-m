...
### Maven repo ordering matters — AppLovin must be last

After applying the SDK integration, the next build failed at runtime classpath resolution:

```log
Could not resolve com.mbridge.msdk.oversea:mbridge_android_sdk:16.9.91.
  Required by: project ':app' > so.daro:daro-m:1.3.4 > com.applovin.mediation:mintegral-adapter:16.9.91.0
  > Could not HEAD '.../artifacts.applovin.com/.../mbridge_android_sdk-16.9.91.pom'.
    Received status code 403 from server: Forbidden
```

Each AppLovin mediation adapter (Mintegral / Ogury / Pangle / Smaato / Chartboost / Verve / IronSource) pulls its underlying network SDK from the _network's own_ Maven repo, not from AppLovin's. AppLovin's repo at `artifacts.applovin.com/android` answers a HEAD on a foreign artifact with `HTTP 403` instead of `404` — and a 403 is fatal to Gradle's resolution loop, stopping the search before later repos are tried.

`youngkeul-rn-app/android/settings.gradle` already encodes this with a comment `// 맨 마지막으로 이동` ("moved to last") next to the AppLovin repo. Applied the same ordering to `example/android/build.gradle`'s `allprojects.repositories` block — full list (omitting `google()` / `mavenCentral()`):

```log
devrepo.kakao.com → android-sdk.is.com → artifact.bytedance.com/repository/pangle →
dl-maven-android.mintegral.com → maven.ogury.co → s3.amazonaws.com/smaato-sdk-releases →
verve.jfrog.io → cboost.jfrog.io/chartboost-ads → cboost.jfrog.io/chartboost-mediation →
artifacts.applovin.com  ← last
```

`buildscript.repositories` only needs `kakao.com` for the `so.daro:daro-plugin` classpath — the network repos are runtime-only.

### Native CMake build: transient `android_gradle_build_mini.json` race on retry

After the runtime classpath resolved, a first clean build failed at `:app:buildCMakeDebug[x86_64]`:

```log
java.io.FileNotFoundException:
  example/android/app/.cxx/Debug/.../x86_64/android_gradle_build_mini.json (No such file or directory)
```

The `mini.json` is generated by `configureCMake` and consumed by `buildCMake`. Under Gradle's parallel task scheduler the two steps for x86_64 can race when other ABIs (`armeabi-v7a`, `x86`, `arm64-v8a`) are running concurrently and consuming shared CMake config state.

Resolution: re-run the same `./gradlew :app:assembleDebug` without `clean`. The configure step is then already complete from the first run, the JSON exists, and the build succeeds. If this race recurs in CI or on slower machines, narrow ABIs in `example/android/gradle.properties`:

```properties
reactNativeArchitectures=arm64-v8a
```

Modern Android devices are arm64-v8a; building all four ABIs is only needed for store-ready release artifacts.
...
2. **`new NativeEventEmitter() ... without required addListener / removeListeners methods`** — fired twice on first listener registration. `src/EventEmitter.ts` constructed `new NativeEventEmitter(NativeModules.DaroMModule)` (the legacy pattern). Under Bridgeless / New Arch, RN's `NativeEventEmitter` expects `addListener` and `removeListeners` to be exposed on the JS module object even though the native `RCTEventEmitter` superclass already implements them.
   ...
   **Regression discovered after v1.1.0 publish**: the host app (`youngkeul-rn-app` on RN 0.79.0, legacy bridge, `newArchEnabled=false`) crashed at module-init with `Cannot read property 'addEventListener' of undefined` on iOS. Inspecting `node_modules/react-native/Libraries/EventEmitter/NativeEventEmitter.js` revealed:
