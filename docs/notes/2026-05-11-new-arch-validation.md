...
The host-app integration that `host-rn-app` already does and that the example was missing:
...
`host-rn-app/android/settings.gradle` already encodes this with a comment `// 맨 마지막으로 이동` ("moved to last") next to the AppLovin repo. Applied the same ordering to `example/android/build.gradle`'s `allprojects.repositories` block — full list (omitting `google()` / `mavenCentral()`):
