...
| `initializeSdk`            | `DaroAds.shared.initialized` triggers a `dispatch_once` that reads `UIApplication.applicationState`; under the New Architecture interop the shared TurboModule queue (not main) executes it, firing Main Thread Checker | SDK entry point wrapped in `DispatchQueue.main.async` so the once-block captures main as its execution thread. See ADR-005 §2 |
| `showMediationDebugger`    | Presents a `UIViewController` synchronously from the bridge queue                                                                                                                                                       | Wrapped in `DispatchQueue.main.async`; `resolve` made `@escaping` accordingly                                                 |
...
| `android/build.gradle`     | Applied `com.facebook.react` plugin under New Arch without a scoping `react { }` block, so codegen scanned the entire consuming workspace's `node_modules` and emitted foreign `*ManagerDelegate` classes (e.g. `RNCSafeAreaProviderManagerDelegate` from `react-native-safe-area-context`) into this library's AAR, causing `:app:mergeLibDexDebug` "Type ... is defined multiple times" failures | Added `react { jsRootDir = file("../src/"); libraryName = "DaroM"; codegenJavaPackageName = "com.darom" }` to scope codegen to this library's own `src/`, which has no codegen sources |
