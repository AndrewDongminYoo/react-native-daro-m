...
class DaroMAdBannerViewManager : SimpleViewManager<DaroMAdBannerViewContainer>() {
  ...
  override fun createViewInstance(
    ...
    val baseKey =
      if (!adUnitId.isNullOrBlank() && !placement.isNullOrBlank()) "${adUnitId}_$placement" else ""
    val cacheKey = if (baseKey.isNotEmpty()) "${baseKey}_${reactContext.hashCode()}" else ""

    if (cacheKey.isNotBlank()) {
      viewCache.keys
        .filter { it.startsWith("${baseKey}_") && it != cacheKey }
